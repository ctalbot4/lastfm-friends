...
async function processMbQueue() {
    ...
    while (true) {
        ...
        try {
            ...
            if (year) {
                self.postMessage({ type: 'result', id, year: parseInt(year, 10), source: album.searchByName ? 'MB-name' : 'MB' });
...
async function processItunesQueue() {
    while (true) {
        ...
        try {
            ...
            if (year) {
                self.postMessage({ type: 'result', id, year: parseInt(year, 10), source: 'iTunes' });
            } else {
